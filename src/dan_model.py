...
class domain_discriminator(nn.Module):
    ...
    def forward(self, inputs):
        return self.output_layer(grad_reverse(inputs))
...
class dann_model():
    ...
    def fit(self, xs, ys, xt, xv, yv,
            epoch=20, batch_size=16, lr=0.001, lr_adv=0.001, early_stop=True, verbose=True):
        ...
        for e in range(epoch):
            ...
            for b, (source_cate, source_num, source_label, target_cate, target_num) in enumerate(train_loader):
                ...
                domain_pred = self.discriminator(torch.cat((source_middle, target_middle)))
