...
from dann_model import dann_model, embed_nn
...
for p in period:
    ...
    for scale in scale_range:
        ...
        for seed in range(10):
            ...
            model = dann_model(embed, cate_index, device)
            ...
            perfs.append(perf)
        perf_scale.append(np.mean(perfs))
    period_scale.append(scale_range[np.argmax(perf_scale)])

path = os.path.join("./results", task, model_type, 
                     "{}_{}".format(model_type, version))
if not os.path.exists(path):
    os.makedirs(path)
np.save(os.path.join(path, "period_scale"), period_scale)

print("Optimal scale for each period:", period_scale, flush=True)
