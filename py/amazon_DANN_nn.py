...
from dann_model import dann_model_num, fully_connected_nn
...
scale_range = [0.01, 0.05, 0.1]
model_scale = {}
for model_domain in ["books", "dvd", "elec", "kitchen"]:
    model_scale[model_domain] = {}
        ...
        if data_domain != model_domain:
            ...
            for scale in scale_range:
                ...
                for seed in range(10):
                    ...
                    model = dann_model_num(dnn, device)
                    ...
                    model.fit(source_train[source_index], source_train_label[source_index], 
                              ...
                              epoch=epoch, batch_size=batch_size, lr=0.005, scale=scale,
                    ...
                    perfs.append(perf)
                perfs_scale.append(np.mean(perfs))
            model_scale[model_domain][data_domain] = scale_range[np.argmax(perfs_scale)]
            
path = os.path.join("./results", task, model_type, 
                     "{}_{}".format(model_type, version))
if not os.path.exists(path):
    os.makedirs(path)
np.save(os.path.join(path, "model_scale"), model_scale)

print("Optimal scale for each period:", model_scale, flush=True)
        ...
        for data_domain in ["books", "dvd", "elec", "kitchen"]:
            if data_domain != model_domain:
                ...
                model.fit(source_train[source_index], source_train_label[source_index], 
                          target_train[target_index],
                          source_valid, source_valid_label,
                          epoch=epoch, batch_size=batch_size, lr=0.005, scale=model_scale[model_domain][data_domain],
                          early_stop=False, verbose=False)
